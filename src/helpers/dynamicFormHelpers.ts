 ...
 * Extracted from dynamicFormController to reduce complexity
 */
 ...
 */
export function isWizardForm(value: unknown): value is WizardForm {
 ...
 */
export function hasFormId(params: Record<string, string>): params is Record<string, string> & { formId: string } {
 ...
 */
export function generateFieldName(question: string): string {
 ...
 */
export async function processFormConfig(fields: FormField[]): Promise<FormField[]> {
 ...
 */
export function extractDateComponentsForValidation(fieldName: string, data: Record<string, string | string[]>): { day: string; month: string; year: string } {
 ...
 */
export function validateDateField(fieldName: string, data: Record<string, string | string[]>): boolean {
 ...
 */
export function validateRegularField(fieldName: string, data: Record<string, string | string[]>): boolean {
 ...
 */
export function validateStepData(config: FormField[], data: Record<string, string | string[]>): {
 ...
 */
export function validateFormRequest(req: RequestWithCSRF, res: Response): string | null {
 ...
 */
export function validateAndGetCurrentStep(wizardForm: WizardForm, stepNumber: number, res: Response): WizardStep | null {
 ...
 */
export function buildFieldNamesList(processedFields: FormField[]): string[] {
 ...
 * @returns {object} Object with day, month, year strings
 */
export function extractDateComponents(fieldName: string, rawFormData: Record<string, unknown>): { day: string; month: string; year: string } {
 ...
 */
export function processDateField(fieldName: string, rawFormData: Record<string, unknown>): Record<string, string> {
 ...
 */
export function processRegularField(fieldName: string, rawFormData: Record<string, unknown>): Record<string, string | string[]> {
 ...
 */
export function extractAndConvertFormData(body: unknown, processedFields: FormField[]): Record<string, string | string[]> {
 ...
 */
export function convertFormDataForSession(formData: Record<string, string | string[]>): Record<string, string> {
 ...
 * Collects data from all steps that have session data stored
 * @param {RequestWithCSRF} req - Express request object
 * @param {string} formId - Form identifier
 * @param {number} totalSteps - Total number of steps in the wizard
 */
export function consolidateFormData(req: RequestWithCSRF, formId: string, totalSteps: number): void {
  const consolidatedData: Record<string, string> = {};
  const FIRST_STEP = 1;
  
  // Collect data from all steps that have session data
  for (let step = FIRST_STEP; step <= totalSteps; step += FIRST_STEP) {
    const stepData = getSessionData(req, `wizardForm_${formId}_step_${step}`) ?? {};
    // Only add if the step has data
    if (Object.keys(stepData).length > EMPTY_ARRAY_LENGTH) {
...
/**
 * Clear all session data for a specific form
 * Used when starting a new form (step 1)
 * @param {Request} req - Express request object
 * @param {string} formId - Form identifier
 */
export function clearFormSessionData(req: Request, formId: string): void {
  const sessionKeys = Object.keys(req.session);
  for (const key of sessionKeys) {
    if (key.startsWith(`wizardForm_${formId}`)) {
